...
class TRTInference:
    ...
    def ping_server(self, host="127.0.0.1", port=5000, endpoint="ping", timeout=5):
        """
        Pings http://{host}:{port}/{endpoint}, parses JSON, and returns
        a Python dict.
        """
        ...
        try:
            with urllib.request.urlopen(url, timeout=timeout) as resp:
                ...
                # Read and decode bytes→str
                raw = resp.read().decode("utf-8")
                # Parse JSON
                data = json.loads(raw)
                return data
        except Exception as e:
            print(f"[ERROR] ping_server failed: {e}")
            return None

    def run(self, image_path: str, output_filename: str = 'trt_output.npy'):
        # --- Preprocess Image ---
