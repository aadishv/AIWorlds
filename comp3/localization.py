...
class OliverLocalization:
    ...
    def run_points_w_theta(self, angles, depths, theta):
        ...
        for p_angle, p_depth in zip(angles, depths):  # point angle, point depth
            ...
            new_coord = wall_amt + (1 if wall in [1, 2] else -1) * np.cos(np.radians(angle)) * p_depth
            if wall in [0, 2]:
        ...
        self.pose = (self.pose[0] + 0.3 * (x - self.pose[0]), self.pose[1] + 0.1 * (y - self.pose[1]), theta)
        # if x_constraints or y_constraints:
        #     exit()
        ...
        for mi in range(measurements.shape[0]):
            ...
            for i in range(1, 640):
                if m[i] <= 0:
                    continue
                angles.append(
                    theta + self.horizontal_angle(i - 320, self.FOCAL_LENGTH))
                depths.append(m[i])
        self.run_points_w_theta(np.array(angles),
                                np.array(depths), theta)
